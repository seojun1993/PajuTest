...
var gl_xml_conf = {
    ...
    url_data:"http://3.35.255.211/user/xml/kiosk_contents.jsp?kiosk_code=KANTH001",
...
// b_code : buildingCode , code : floorCode, name : 키오스크 화면 층 이름
// var gl_arr_floors = new Array(
//     {"b_code":"B0001","code":"전체지도","name":"전체지도"},
//     {"b_code":"B0002","code":"지하지도","name":"지하지도"},
//     {"b_code":"B01","code":"1F","name":"1F"},
//     {"b_code":"B01","code":"M1F","name":"M1"},
//     {"b_code":"B01","code":"2F","name":"2F"},
//     {"b_code":"B01","code":"3F","name":"3F"},
//     {"b_code":"B01","code":"4F","name":"4F"},
//     {"b_code":"B01","code":"5F","name":"5F"},
//     {"b_code":"B01","code":"6F","name":"6F"},
//     {"b_code":"B01","code":"RF","name":"RF"},
// );
...
// gl_arr_floors.push(obj);
// console.log('gl_arr_floors', gl_arr_floors);
...
var gl_final_map_pos = {
    "TWOWAY":{"pos0":{"xx":100, "yy":-350, "ss":0.25 },"pos1":{"xx":100, "yy":170, "ss":0.25}},
    "THREEWAY":{"pos0":{"xx":-225, "yy":-375, "ss":0.2 },"pos1":{"xx":225, "yy":-375, "ss":0.2},"pos2":{"xx":0, "yy":300, "ss":0.2}},
    "FOURWAY":{"pos0":{"xx":-225, "yy":-375, "ss":0.2 },"pos1":{"xx":225, "yy":-375, "ss":0.2},"pos2":{"xx":-225, "yy":300, "ss":0.2 },"pos3":{"xx":225, "yy":300, "ss":0.2}}
};
...
function setInitConfig(p_load_data){
    ...
    var option = {
        ...
        map_scale:"auto",
        img_curr_type:{"url":"images/wayfind/floor_minimap_start.svg", "width":"40", height:"56"},
        img_target_type:{"url":"images/wayfind/floor_minimap_arrival.svg", "width":"60", height:"60"},
        img_move_type:{"url":"images/wayfind/ico_m_point_human.png", "width":"48", height:"48"},
        disp_background:"rgba(239, 239, 246, 0)"
    };


    /*for ( i = 0; i<gl_map_list.length; i++) {


        var obj = {
            building_code : gl_map_list[i].KIOSK_BUILDING,
            floor_code : gl_conf_header.KIOSK_FLOOR.text,
            name : gl_map_list[i].floor
        }
    ...
    for(i = 0; i < gl_arr_floors.length; i++){
        if(gl_arr_floors[i].name == gl_conf_header.KIOSK_FLOOR){
            i_found = i;
            break;
        }
    ...
    // STORE 리스트 없어서 잠시 주석 처리 - 민정(0627)
...
function setMainLang(p_type, p_lang){
    ...
    if(gl_main_conf.map_load == "SUCC"){
        var send_param = {"sect":"CMD","type":"LANG","code":gl_main_conf.lang};
    ...
    if(p_obj != null && p_obj.sect != undefined){
        ...
        if(p_obj.sect == "LOCATION"){
            if(p_obj.type == "STORE"){
                if(gl_main_conf.map_load == "SUCC"){
                    is_wayfind = true;
                    var send_param = {"sect":"WAYFIND","type":"STORE","id":p_obj.id,"way_type":"LOCATION"};
                    gl_floor_map.setCallWebToMap(send_param);
                }
            }else{
                is_wayfind = true;
                if(gl_main_conf.map_load == "SUCC"){
                    var send_param = {"sect":"WAYFIND","type":"PUB","id":p_obj.id};
                    gl_floor_map.setCallWebToMap(send_param);
                }
            }
            ...
            if(p_obj.type == "STORE"){
                if(gl_main_conf.map_load == "SUCC"){
                    is_wayfind = true;
                    var send_param = {"sect":"WAYFIND","type":"STORE","id":p_obj.id,"way_type":"WAYFIND","move_type": getChkNull(p_obj.move_type,"")};
                    gl_floor_map.setCallWebToMap(send_param);
                }
            }else if(p_obj.type == "PARK"){
                if(gl_main_conf.map_load == "SUCC"){
                    is_wayfind = true;
                    var send_param = {"sect":"WAYFIND","type":"PARK","id":p_obj.id,"way_type":"WAYFIND","near_type":"TARGET","move_type": getChkNull(p_obj.move_type,"")};
                    gl_floor_map.setCallWebToMap(send_param);
                }
            }
        }
    ...
    for(i = 0; i < gl_arr_floors.length; i++){
        ...
        if(gl_arr_floors[i].b_code == p_b_code && gl_arr_floors[i].code == p_floor){
            ...
            if(p_type == "INIT" || p_type == "LOCATION"){
                ...
                if(gl_main_conf.map_load == "SUCC"){
                    var send_param = {"sect":"CMD","type":"FLOOR","b_code":p_b_code,"floor":p_floor,"move_type":p_type};
                    gl_floor_map.setCallWebToMap(send_param);
...
function onCallBackElovMap(p_evt){
    ...
    if(detail.sect == "INIT"){
        ...
        if(detail.result == "SUCC"){
            ...
            var cmd_obj = { sect:"FLOOR", type:"DONE"};
        ...
        var cmd_obj = { sect:"POPUP", type:"STORE_INFO", id:detail.id, code:"" };
        if(parent.MAINPARENTCUSTOMCODE){ parent.setParentCmd(cmd_obj); }
        ...
        if(detail.result == "NOWAY"){ // 길이 없음
            var cmd_obj = { sect:"POPUP", type:"ERROR", msg:"길을 찾을 수가 없습니다" };
            if(parent.MAINPARENTCUSTOMCODE){ parent.setParentCmd(cmd_obj); }
        }else if(detail.result == "NONE"){ // 정보를 찾을수 없음
            var cmd_obj = { sect:"POPUP", type:"ERROR", msg:"정보가 없습니다." };
            if(parent.MAINPARENTCUSTOMCODE){ parent.setParentCmd(cmd_obj); }

        }else if(detail.result == "WAYTYPE"){  // 길안내 선택
            if(detail.type == "STORE"){
                var cmd_obj = { sect:"POPUP", type:"STORE_WAYTYPE",id:detail.id,target_floor:detail.target_floor,target_x:detail.target_x,target_y:detail.target_y};
                if(parent.MAINPARENTCUSTOMCODE){
                    parent.setParentCmd(cmd_obj);
                }
            }else if(detail.type == "PARK"){
                var cmd_obj = { sect:"POPUP", type:"PARK_WAYTYPE",id:detail.id,target_floor:detail.target_floor,target_x:detail.target_x,target_y:detail.target_y};
                if(parent.MAINPARENTCUSTOMCODE){
                    parent.setParentCmd(cmd_obj);
                }
            }
...
function onClickPubInfo(p_obj){
    ...
    //if(gl_main_conf.map_load == "SUCC"){
    var send_param = {"sect":"WAYFIND","type":"PUB","way_type":"NEARLOCATION","id":str_code};
    gl_floor_map.setCallWebToMap(send_param);
    //}

    console.log("str_code : " ,str_code)
    var statics_obj = {"sect":"PUBLIC","code":str_code};
    var cmd_obj = {
        sect: "STATICS",
        type: "",
        id: "",
        code: statics_obj.code,
        obj:{sect:"PUBLIC",code:statics_obj.code}
    };
    parent.setParentCmd(cmd_obj);
}
...
function onClickDebugWay(p_id){
    ...
    if(gl_main_conf.map_load == "SUCC"){
        for(i = 0; i < gl_arr_store_list.length; i++){
            ...
            if(obj.ID == p_id){
                console.log(obj);
                var send_param = {"sect":"WAYFIND","type":"STORE","id":p_id,"way_type":"WAYFIND","move_type":"ESC"};
                gl_floor_map.setCallWebToMap(send_param);
                break;
            }
        }
...
function onClickDebugPark(p_id){
    if(gl_main_conf.map_load == "SUCC"){
        var send_param = {"sect":"WAYFIND","type":"PARK","id":p_id,"way_type":"WAYFIND","move_type":"ELE"};
        gl_floor_map.setCallWebToMap(send_param);
    }
}
...
function onClickDebugLoc(p_id){
    if(gl_main_conf.map_load == "SUCC"){
        var send_param = {"sect":"WAYFIND","type":"STORE","store_id":p_id,"way_type":"LOCATION","move_type":"ELE"};
        gl_floor_map.setCallWebToMap(send_param);
    }
}
...
function onClickDebugPub(p_code){
    if(gl_main_conf.map_load == "SUCC"){
        if(p_code == "P24"){
            var send_param = {"sect":"WAYFIND","type":"PUB","way_type":"NEARLOCATION","id":p_code};
            gl_floor_map.setCallWebToMap(send_param);
        }else{
            var send_param = {"sect":"WAYFIND","type":"PUB","way_type":"NEARLOCATION","id":p_code};
            gl_floor_map.setCallWebToMap(send_param);
...
function onClickDebugTwoWay(){
    if(gl_main_conf.map_load == "SUCC"){
        var send_param = {"sect":"DEBUG","type":"TWOWAY"};
        gl_floor_map.setCallWebToMap(send_param);
    }
}
...
function onClickDebugMove(p_type){
    if(gl_main_conf.map_load == "SUCC"){
        if(p_type == "A"){
            var xx = (0 - 340);
            var yy = (0 - 1140);
            var send_param = {"sect":"CMD","type":"MOVE","x":xx,"y":yy};
            gl_floor_map.setCallWebToMap(send_param);
        }else{
            var send_param = {"sect":"CMD","type":"MOVE","x":1300,"y":700};
            gl_floor_map.setCallWebToMap(send_param);
        }
    }
}
...
function onClickDebugScale(p_type){
    if(gl_main_conf.map_load == "SUCC"){
        if(p_type == "PLUS"){
            var send_param = {"sect":"CMD","type":"SCALE","scale":"IN"};
            gl_floor_map.setCallWebToMap(send_param);
        }else{
            var send_param = {"sect":"CMD","type":"SCALE","scale":"OUT"};
            gl_floor_map.setCallWebToMap(send_param);
        }
    }
}
...
function onClickDebugLine(){
    var send_param = {"sect":"DEBUG","type":"LINE"};
    gl_floor_map.setCallWebToMap(send_param);
}