...
function setInitSetting(p_result) {
	...
	document.getElementById("id_list_area").addEventListener(
		"touchstart",
		function (evt) {
	...
	$("#id_search_type_floor li").mousedown(function () {
...
function setInitConfig(p_load_data) {
	...
	gl_arr_store_list.sort(function (a, b) {
		// 오름차순
...
function setMainStart(p_obj) {
	...
	$("#id_search_type_floor li > span").removeClass("active");
...
function setInitMakeStoreList() {
	...
	for (i = 0; i < gl_arr_store_list.length && i < 3000; i++) {
		...
		str_html = "<div id='id_store_box_" + obj.ID + "' class='info' style='transform:translate(0px,0px);'> ";
		...
		str_html += "               </div>";
		str_html += "                   <div class='floor_txt'>" + obj.STORE_NAME_KOR + "</div>";
		str_html += "           </div>";
		str_html += "           <div class='desc_bottom'>";
		str_html += "               <div class='left_area'>";
		str_html += "                   <div class='floor_input'>" + obj.STORE_FLOOR + "</div>";
		str_html += "               </div>";
...
function onClickSelectType(p_obj) {
	...
	if (str_code == "KEYWORD") {
		...
		onClickSearchFloor($("#id_search_type_floor li")[0]);
	}
}
...
function onClickSearchCateMain(p_obj) {
	...
	for (i = 0; i < i_len; i++) {
		...
		str_cate_code = $(obj).attr("cate_code");
		if (str_code == str_cate_code) {
			i_cnt++;
			$(obj).show();
		} else {
			$(obj).hide();
...
function onClickSearchFloor(p_obj) {
	var str_code = $(p_obj).attr("code");
	$("#id_search_type_floor li").removeClass("active");
	$(p_obj).addClass("active");
...
// 상품 상세보기 클릭
function onClickStoreInfo(p_evt, p_id) {
