...
function setInitConfig(p_load_data) {
    ...
    gl_conf_time = p_load_data.time_obj;

    var notice_list = p_load_data.arr_notice_list;
    //console.log("notice_list " + notice_list.length);

    for (i = 0; i < notice_list.length; i++) {
        str_sday = notice_list[i].SDAY + "";
        str_eday = notice_list[i].EDAY + "";
        i_sday = parseInt(str_sday);
        i_eday = parseInt(str_eday);

        if (i_sday <= i_date && i_eday >= i_date) {
            gl_arr_notice_list.push(notice_list[i]);
        }
    }

    var intro_bg_list = p_load_data.arr_intro_bg_list;
    //console.log("DDDDDDDDD",intro_bg_list[0]);
    $("#id_list_bg > img").attr("src", intro_bg_list[0]);
    //intro_bg_list
    //console.log("gl_arr_notice_list " + gl_arr_notice_list.length);
    //setMakeIntroList();
...
function setMainInterval() {
    ...
    let t_list = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
...
function setMainLang(p_type, p_lang) {
    ...
    $('#id_line1').html(gl_conf_time["INTRO_LINE1_"+p_lang]);    
    $('#id_line2').html(gl_conf_time["INTRO_LINE2_"+p_lang]);    
    $('#id_line3').html(gl_conf_time["INTRO_LINE3_"+p_lang]);    
    $('#id_line4').html(gl_conf_time["INTRO_LINE4_"+p_lang]);    
    $('#id_line5').html(gl_conf_time["INTRO_LINE5_"+p_lang]);
