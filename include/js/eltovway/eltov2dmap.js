...
class Eltov2DMap{
    ...
    setCallWebToMap(p_param){
        ...
        if(p_param.sect == "CMD"){
            if(p_param.type == "LANG"){
                ...
                this.setChgFloor(this.m_util.getChkNull(p_param.move_type,"LOCATION"),this.m_util.getChkNull(p_param.b_code,"B0001"),p_param.floor);
    ...
    setChgFloor(p_type,p_b_code,p_floor){
        ...
        if(p_b_code == undefined || p_b_code == "") p_b_code = "B0001";
        ...
        if(this.m_wayfind_conf.move_cnt < this.m_wayfind_conf.arr_floors.length){
            ...
            if(this.m_wayfind_conf.mode == "ONEWAY"){
                ...
                if(this.m_wayfind_conf.arr_floors.length == 2){  // 길안내가 2개 일 경우
                    ...
                    //yy = 0;
                    xx = this.m_conf.final_map_type["TWOWAY"]["pos0"].xx;
                    yy = this.m_conf.final_map_type["TWOWAY"]["pos0"].yy;
                    ss = this.m_conf.final_map_type["TWOWAY"]["pos0"].ss;
                    //xx = 100;
                    //yy = -350;
                    ...
                    //xx = this.m_conf.disp_width * (1/4);
                    //yy = 0;
                    xx = this.m_conf.final_map_type["TWOWAY"]["pos1"].xx;
                    yy = this.m_conf.final_map_type["TWOWAY"]["pos1"].yy;
                    ss = this.m_conf.final_map_type["TWOWAY"]["pos1"].ss;
                    //xx = 100;
                    //yy = 170;
                    ret_pos = this.m_util.getPosTransform(obj_floor.getMainDisp());
                    obj_floor.setMainDispPos((0 - this.m_conf.map_width/2),(0 - this.m_conf.map_height/2),-1);
    ...
    setChkOpt(p_opt){
        ...
        this.m_conf.url_data =  p_opt.url_data || "http://3.35.255.211/user/xml/kiosk_contents.jsp?kiosk_code=KANTH001";
        this.m_conf.url_route =  p_opt.url_route || "http://3.35.255.211/user/xml/kiosk_route.jsp?brn_code=ANT";
